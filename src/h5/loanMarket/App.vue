<template>
  <div class="loanMarket">
    ...
    <!-- 下拉加载动图 -->
    <refresh-load v-show="isRefresh"></refresh-load>
  </div>
</template>
...
<script>
  ...
  import refreshLoad from 'components/refreshLoad/index';// 下拉刷新加载动图
  import { getUrlPara, alertMsg, nativeAppFun, linkToLogin, userAgent } from 'assets/js/utils';
  ...
  export default {
    ...
    methods:{
      ...
      jumpLink(link, id, pointSubCode, obj){//链接跳转
        if(!link && !id) {
          return;
        }
        //判断是否登录，否则前去登录
